...
class SecretManagerService {
    ...
    async addPageToken(pageId, token, expiresIn) {
        ...
        const projectId = utils_1.config.gcloud.projectId || process.env.GCP_PROJECT_ID || process.env.GOOGLE_CLOUD_PROJECT;
        if (!projectId)
            throw new Error('GCP project ID not configured (set GCP_PROJECT_ID/GOOGLE_CLOUD_PROJECT or config.gcloud.projectId)');
