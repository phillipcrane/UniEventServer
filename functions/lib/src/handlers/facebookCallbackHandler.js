...
async function handleCallback(deps, req, res) {
    ...
    if (!code) {
        return utils_1.HttpStatusUtil.send(res, 400, 'Missing code');
    }
    try {
        ...
        if (!Array.isArray(pages) || pages.length === 0) {
            return utils_1.HttpStatusUtil.send(res, 200, 'No pages returned.');
        }
        ...
        if (String(req.query.debug) === '1') {
            return utils_1.HttpStatusUtil.send(res, 500, msg);
        }
        utils_1.HttpStatusUtil.send(res, 500, 'Facebook auth failed');
    }
}
