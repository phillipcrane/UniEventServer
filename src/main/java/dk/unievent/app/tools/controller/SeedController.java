...
public class SeedController {
    ...
    public SeedController(SeedService seedService, @Value("${unievent.tools.admin-key:}") String adminKey) {
    ...
    @ApiResponse(responseCode = "401", description = "Missing or invalid X-Admin-Key header")
    @ApiResponse(responseCode = "500", description = "Error during seeding")
    public ResponseEntity<?> seed(@RequestHeader(value = "X-Admin-Key", required = false) String providedKey) {
        if (!isAuthorized(providedKey)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body("Unauthorized: please request the .env file from the dev team");
        }
    ...
    public ResponseEntity<?> clear(@RequestHeader(value = "X-Admin-Key", required = false) String providedKey) {
        if (!isAuthorized(providedKey)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body("Unauthorized: please request the .env file from the dev team");
        }
